...
7. Right click new cert in Keychain Access and export as `p12` with a password, save it to `credentials/ios/dist-cert.p12`
8. Identifiers -> Create new app ID
9. Create new App Store Connect provisioning profile, download `.mobileprovision` file and move it to `credentials/ios/profile.mobileprovision`
...
3. Create Team key
4. Role: *Developer*
5. Download key p8 and save it to `~/.appstoreconnect/private_keys/AuthKey_*.p8`
6. Copy Issuer ID, Key ID
